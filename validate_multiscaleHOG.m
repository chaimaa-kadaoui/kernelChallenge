function [acc, mean_acc] = validate_multiscaleHOG(x_train_total,y_train_total, kernel, p_norm)

    %% Preprocess data

    x_train_total_p = process_multiscaleHOG(x_train_total, p_norm);
    ...
    for valPart=1:splitPart
        ...
        % stop validation on a bad model
        if acc<0.39
    ...
    fprintf('Average accuracy with kernel %s and normalization %i: %f\n', kernel, p_norm, mean_acc);
end